...
async function handleEnable(row) {
  ...
  try {
    await api.manageupdateStatus({ id: row.id, status: row.status == 1 ? 0 : 1 })
...
function handleOpenRolesSet(row) {
  ...
  if(row.id == userStore.userId) {
    $message.warning('不能修改自己的等级')
    return
  }
  if(row.manage_type >= userStore.userInfo?.manage_type) {
    $message.warning('只能修改自己下级管理员的等级')
    return
  }
...
function resetPwd(row) {
  const d = $dialog.warning({
    ...
    async onPositiveClick() {
      try {
        d.loading = true
        const data = await api.manageresetPwd({id: row.id})
...
const {
  ...
  doCreate: api.managecreate,
  doDelete: api.managedelete,
  doUpdate: api.manageupdate,
...
function onSave() {
  if (modalAction.value === 'setRole') {
    return handleSave({
      api: () => api.manageupdate(modalForm.value),
      cb: () => $message.success('分配成功'),
    })
  } else if (modalAction.value === 'reset') {
    return handleSave({
      api: () => api.manageresetPwd(modalForm.value.id, modalForm.value),
