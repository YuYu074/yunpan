            ...
            <el-container>
                ...
                <el-main>
                            ...
                            <el-dialog title="请填写信息" :visible.sync="dialogFormVisible">
                                <el-form :model="form">
                                    <el-form-item label="座位号" :label-width="formLabelWidth">
                                        <el-input v-model="form.name" autocomplete="off" placeholder="请输入座位号"></el-input>
                                    </el-form-item>
                    ...
                    <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName">
                        ...
                        <el-table-column prop="name" label="座位号" width="150">
                        </el-table-column>
                        <el-table-column prop="type" label="类型" width="70">
                              <template slot-scope="scope">
                      {{ scope.row.type == 0 ? '普通座' : '沉浸式' }}
                    </template>
                        </el-table-column>
                        <el-table-column prop="status" label="使用情况" width="150">
                             <template slot-scope="scope">
...
export default {
    methods: {
        addItem() {
            // 向list新增item
            this.$http.post("/admin/seat/save", this.form)
           .then(response => {
        ...
        deleteUser(id) {
            this.$http.delete(`/admin/seat/${id}`).then((response) => {
                    //request success handler
                    //const message = JSON.stringify(response.data.msg) */// 假设后端返回数据格式为 {message: '添加数据成功'}
                    if (response.data.code === 1)
                     {
       ...
       /*  handleClick(row) {
        ...
        handleQuery() {
            this.$http.post('/admin/seat/query', this.form).then((Response) => {
